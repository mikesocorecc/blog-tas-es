...
if (!class_exists('WordpressHttpResponse')) require_once('wordpress_http_response.php');
...
class OAuthApplication implements AWeberOAuthAdapter {
    ...
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'AWeberWebFormPluginNamespace\OAuthServiceProvider')) {
    ...
    protected function http_get($url, $data) {
        $url = $this->_addParametersToUrl($url, $data);
        return $this->get_api_response(wp_remote_get($url));
    }
    ...
    private function get_api_response($response) {
        ...
        $this->error = $response->get_error_code() . ' - '
     ...
     * post
     *
     * Prepare an OAuth post method and make a Http Post Request.
     *
     * @param mixed $url    URL where we are making the request to
     * @param mixed $data   Data that is used to make the request
     * @access protected
     * @return Object
     */
    protected function http_post($url, $oauth) {
        $postData = $this->buildData($oauth);
        $args = array(
            'body' => $postData
        );
        return $this->get_api_response(wp_remote_post($url, $args));
    }
